...
vector<string> GPX2::crossover(vector<string> redT, vector<string> blueT)
{
    ...
    {
        std::ofstream map;
        map.open("red_semGhost.map");
        obj.printMap(obj.red, map);
        map.close();   
    }
    {
        std::ofstream map;
        map.open("blue_semGhost.map");
        obj.printMap(obj.blue, map);
        map.close();   
    }
    ...
    {
        std::ofstream map;
        map.open("KaprielViado.map");
        obj.printMap(obj.unitedGraph, map);
        map.close();
    }
    ...
    if (obj.offspringChoosen == Parent::RED) {
        ...
        // Step 9
        offspring = obj.mapToTour(obj.red);
    } else {
...
{ // Mapear o tour para um grafo com ligações entre os nós
    ...
    for (unsigned i = 1; i < t.size(); i++) { // percorre o vetor a partir do segundo elemento, o primeiro já foi transformado
        ...
        cn->addEdge(CustomerNode::node(prev->getId(), dist)); // adiciona ao node atual as arestas de conexão

        prev->addEdge(CustomerNode::node(cn->getId(), dist)); // adiciona ao node anterior o atual como um próx (lista duplamente encadeada)
    ...
    prev->addEdge(CustomerNode::node(first->getId(), dist)); // o atual recebe o primeiro para completar os ligamentos
    ...
    for (auto& city : red) {
        ...
        if (idKey.find(ghostToken) == string::npos) { // the node is not a ghost
            ...
            if (isGhost.size() == 4) { // node with degree 4
                ...
                CustomerNode* ghostNode = new CustomerNode(ghostID);
                insertGhost(idKey, red, ghostNode);

                ghostNode = new CustomerNode(ghostID);
...
void GPX2::insertGhost(string& id, CustomerGraph& tour, CustomerNode* ghost)
{
    ...
    tour[id]->addEdge(CustomerNode::node(ghost->getId(), 0)); // adiciona ao node "REAL" o edge de ligação com o ghost

    for (unsigned i = 0; i < 2; i++) {
        // percorre os edges de ligação com o nó que foi para o ghost
        CustomerNode::node edge = tour[edgeFirst.first]->getEdges()[i];
        ...
        if (edge.first.compare(id) == 0) { // verifica se é a ligação
            ...
            tour[edgeFirst.first]->addEdge(CustomerNode::node(ghost->getId(), edgeFirst.second));
        }
    }

    ghost->addEdge(edgeFirst); // faz o ghost se ligar ao edge
    ghost->setAccess(true);
    ghost->addEdge(CustomerNode::node(tour[id]->getId(), 0)); // adiciona o REAL como um edge do ghost
...
{ // executa a união dos dois grafos, gerando Gu
    ...
    for (string id : cityList) {
        CustomerNode* c = red[id];
        // criar a entrada no map da união
        unitedGraph.insert(
            make_pair((c->getId()), new CustomerNode((c->getId()))));
        // colocar as edges no map da união
        for (CustomerNode::node n : red[(c->getId())]->getEdges()) {
            unitedGraph[(c->getId())]->addEdge(make_pair(n.first, n.second));
        }
        for (CustomerNode::node n : blue[(c->getId())]->getEdges()) {
            unitedGraph[(c->getId())]->addEdge(make_pair(n.first, n.second));
        }
    }
}
...
{ // a partir do valor passado irá executar uma busca em  Gu' para encontrar as partições nodeOne funciona como uma raiz, que será o ponto inicial da busca, achando  a partição onde ele está contido retorna um vetor com os IDs das cidades que compoem a partição
    ...
    CustomerNode* root; // nó para verificação
    ...
    for (auto& allP : feasiblePartitions) {
        if (partitionsChoosen[index] == Parent::BLUE) { // se o Blue for melhor que o Red naquela partição
            for (string s : allP.second->getNodes()) {
                ...
                CustomerNode* newNode = new CustomerNode(blue[s]->getId());
            ...
            for (string s : allP.second->getNodes()) {
                ...
                CustomerNode* newNode = new CustomerNode(red[s]->getId());
...
void GPX2::removeGhosts(CustomerGraph& graph)
{
   for(CustomerGraph::iterator node=graph.begin(); node!=graph.end(); ){
        ...
        if (index != string::npos) {
            ...
            vector<CustomerNode::node> edges = graph[id]->getEdges();
...
vector<string> GPX2::mapToTour(CustomerGraph& mapOffspring)
{ // Map para tour
    //Tour offspring;
    vector<string> offspring;
    ...
    CustomerNode* city = mapOffspring.begin()->second; // Reduzir chamadas (CustomerNode dentro do map)
    ...
    offspring.push_back(city->getId()); // já foi visitado então entra no filho
    ...
    while (!nextToVisit.empty()) {
        ...
        offspring.push_back(city->getId());

        for (CustomerNode::node n : city->getEdges()) {
...
pair<GPX2::SearchResult, vector<string>> GPX2::DFS_inside(string entry, string exit, CustomerGraph father, Partition* partitionPtr)
{
    ...
    while (!nextToVisit.empty()) {
        ...
        nextToVisit.pop_back();
        alreadyVisited.push_back(now);
        vector<CustomerNode::node> edges = father[now]->getEdges();
...
void GPX2::printMap(CustomerGraph& graph, std::ostream& stream)
{
    ...
    CustomerNode* city = graph.begin()->second; // Reduzir chamadas (CustomerNode dentro do map)
...
{ // Distancia total para percorrer o grafo
    ...
    CustomerNode *city = graph.begin()->second, *last = nullptr; // Reduzir chamadas (CustomerNode dentro do map)
    isAlreadyVisited.push_back(graph.begin()->first);
    ...
    for (CustomerNode::node n : last->getEdges()) {
...
bool GPX2::validatePartition(){
    for(auto p : feasiblePartitions){
        for(string node : p.second->getNodes()){
            int count{0};
            for(auto p1 : feasiblePartitions){
                if(std::find(p1.second->getNodes().begin(), p1.second->getNodes().end(), node)!=p1.second->getNodes().end()){
                    count++;
                }
            }
            for(auto p1 : unfeasiblePartitions){
                if(std::find(p1.second->getNodes().begin(), p1.second->getNodes().end(), node)!=p1.second->getNodes().end()){
                    count++;
                }
            }   
