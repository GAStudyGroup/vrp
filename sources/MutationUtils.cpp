...
bool checkInSameTour(vector<int> Tour, int depotId, int customer1,int customer2){
 ...
 void printVector(vector<int> tour,Map& map){
...
pair<vector<int>,bool> basicFitnessEvaluation(vector<int> OldTour,vector<int> NewTour,Map& map){
    pair<vector<int>,bool> result;
    result.first=OldTour;
    result.second=false;
    if(getTourFitness(NewTour,map)>getTourFitness(OldTour,map)){
        result.first=NewTour;
        result.second=true;
    }
    return result;
}
