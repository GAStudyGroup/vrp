...
vector<string> GPX2Support::mapToTour(GPX2Structs::NodeMap& mapOffspring)
{
    ...
    offspring.push_back(city->getId()); // begin the linearization by the first node of the map and inserts it in the offspring
...
GPX2Structs::NodeMap GPX2Support::tourToMap(vector<string> &t)
{
    ...
    for (unsigned i = 0; i < size; i++) {
        ...
        if(i==0){
            ...
            cn->getEdges().push_back(Node::node(cities[size-1], dist));
            dist = distance(cities[i],cities[i+1]);
            cn->getEdges().push_back(Node::node(cities[i+1], dist));
        }else if(i==(size-1)){
            ...
            cn->getEdges().push_back(Node::node(cities[0], dist));
        }else{
