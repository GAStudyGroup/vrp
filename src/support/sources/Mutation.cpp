...
vector<int> Mutation::evaluateMutation(vector<int> tour){
    ...
    if(value!=Configs::customerMap.getDepotId()){ //Only does the move it isn't a depot   
        for(unsigned int i=0;i<initial.size()-1;i++){
            ...
            vector<int> aux(initial.begin(),initial.end());
            result=basicFitnessEvaluation(Tour(tour),Tour(aux));
            // printVector(initial); //Debug
            if(result.second){
                break;
            }
...
pair<vector<int>,bool> Mutation::secondAndThirdMove(vector<int> tour, int it,int value,int mode){
    ...
    pair<deque<int>,bool> result;
    result.first= deque<int>(tour.begin(),tour.end());
    result.second=false;
    int valU=0;
    int valX=0;
    if(it==Configs::customerMap.getDepotId() || it==(int)tour.size()-1){
        ...
        for(unsigned int i=0;i<result.first.size()-1;i++){
                ...
                auto auxP=basicFitnessEvaluation(Tour(tour),Tour(result.first));
                result.first=deque<int>(auxP.first.begin(),auxP.first.end());
                result.second=auxP.second;
                if(result.second){
                    return make_pair(vector<int>(result.first.begin(),result.first.end()),true);
                }
                result.first=deque<int>(tour.begin(),tour.end());
        }
        result.second=false;
    ...
    if(it<(int)tour.size()-1){
        ...
        if(value!=Configs::customerMap.getDepotId()){
            for(unsigned int i=0;i<tour.size()-1;i++){
                ...
                //printVector(result.first);
                auto aux=basicFitnessEvaluation(Tour(tour),Tour(result.first));
                result.first=deque<int>(aux.first.begin(),aux.first.end());
                result.second=aux.second;
                if(result.second){
                    return make_pair(vector<int>(aux.first.begin(),aux.first.end()),true);
                }
                result.first=deque<int>(tour.begin(),tour.end());
            }
        }
